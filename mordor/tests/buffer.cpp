...
MORDOR_UNITTEST(Buffer, visitSingleSegment)
{
    Buffer b("hello");
    int sequence = 0;
    b.visit(std::bind(&visitor2, std::placeholders::_1,
		std::placeholders::_2, std::ref(sequence)));
...
MORDOR_UNITTEST(Buffer, visitMultipleSegments)
{
    ...
    b.copyIn("a");
    b.copyIn("bc");
	b.visit(std::bind(&visitor3, std::placeholders::_1,
		std::placeholders::_2, std::ref(sequence)));
...
MORDOR_UNITTEST(Buffer, visitMultipleSegmentsPartial)
{
    ...
    b.copyIn("bcd");
	b.visit(std::bind(&visitor3, std::placeholders::_1,
		std::placeholders::_2, std::ref(sequence)), 3);
    MORDOR_TEST_ASSERT_EQUAL(++sequence, 3);
}
